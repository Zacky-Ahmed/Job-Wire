...
/* THE PASSWORD-RESET HAPPY PATH, which had never been tested end to end.
 *
 * The suite deliberately avoided it because it did not want to send
 * mail, so a guaranteed bug lived here undetected: setPassword()
 * incremented sessionVersion in Mongo, and /reset then stamped the
 * session it had just created with the version from the user document
 * loaded BEFORE the write. Database said 1, session said 0, and
 * requireAuth destroyed the brand-new session on the very next request.
 *
 * A successful password reset signed the person straight back out.
 * Every time.
 *
 * No mail is needed to test it: the code is generated by otp.issue()
 * and stored as a bcrypt hash, so the test issues its own and writes the
 * hash exactly as the app would.
 */
const otpMod = await import("../src/services/auth/otp.js");

const resetEmail = `e2e-reset-${Date.now()}@example.invalid`;
const oldPass = "correcthorsebattery";
const newPass = "a-completely-different-one";
const resetUser = (await collections.users().insertOne({
  email: resetEmail, passHash: await pw.hash(oldPass),
  verified: true, verifiedAt: new Date(), createdAt: new Date(),
  sessionVersion: 0,
})).insertedId;

/* An OLD session, which must stop working — the whole point of bumping
   the version is that a stolen cookie dies when the owner resets. */
const oldJar = new Map();
const oldCookie = () => [...oldJar].map(([k, v]) => `${k}=${v}`).join("; ");
const oldStore = (res) => {
  for (const c of res.headers.getSetCookie?.() ?? []) {
    const [p] = c.split(";"); const i = p.indexOf("=");
    oldJar.set(p.slice(0, i), p.slice(i + 1));
  }
};
let r2 = await fetch(BASE + "/signin", { headers: { cookie: "" }, redirect: "manual" });
oldStore(r2);
let h2 = await r2.text();
r2 = await fetch(BASE + "/signin", {
  method: "POST", redirect: "manual",
  headers: { cookie: oldCookie(), "content-type": "application/x-www-form-urlencoded" },
  body: new URLSearchParams({ _csrf: csrf(h2), email: resetEmail, password: oldPass }),
});
oldStore(r2);
ok(r2.status === 302, "the old session signs in before the reset");
const oldWorksBefore = await fetch(BASE + "/wire", { headers: { cookie: oldCookie() }, redirect: "manual" });
ok(oldWorksBefore.status === 200, "and reaches the wire");

/* Now the reset. The code is issued here and its hash written exactly
   as the app writes it, so no mail provider is involved. */
const issued = await otpMod.issue();
await collections.users().updateOne(
  { _id: resetUser },
  { $set: { resetHash: issued.hash, resetExpiresAt: issued.expiresAt, resetAttempts: 0 } }
);

const resetJar = new Map();
const resetCookie = () => [...resetJar].map(([k, v]) => `${k}=${v}`).join("; ");
const resetStore = (res) => {
  for (const c of res.headers.getSetCookie?.() ?? []) {
    const [p] = c.split(";"); const i = p.indexOf("=");
    resetJar.set(p.slice(0, i), p.slice(i + 1));
  }
};

// /forgot establishes the resetUserId the /reset page needs.
let rr = await fetch(BASE + "/forgot", { headers: { cookie: "" }, redirect: "manual" });
resetStore(rr);
let hh = await rr.text();
rr = await fetch(BASE + "/forgot", {
  method: "POST", redirect: "manual",
  headers: { cookie: resetCookie(), "content-type": "application/x-www-form-urlencoded" },
  body: new URLSearchParams({ _csrf: csrf(hh), email: resetEmail }),
});
resetStore(rr);

// The code was consumed by that request issuing a fresh one, so write
// ours back over it and submit.
await collections.users().updateOne(
  { _id: resetUser },
  { $set: { resetHash: issued.hash, resetExpiresAt: issued.expiresAt, resetAttempts: 0 } }
);
hh = await (await fetch(BASE + "/reset", { headers: { cookie: resetCookie() }, redirect: "manual" })).text();
rr = await fetch(BASE + "/reset", {
  method: "POST", redirect: "manual",
  headers: { cookie: resetCookie(), "content-type": "application/x-www-form-urlencoded" },
  /* The form posts SIX single-character fields, not one "code", and
     the confirm box is password2. Getting that wrong is how the first
     run of this test reported four failures against working code. */
  body: new URLSearchParams({
    _csrf: csrf(hh),
    d1: issued.code[0], d2: issued.code[1], d3: issued.code[2],
    d4: issued.code[3], d5: issued.code[4], d6: issued.code[5],
    password: newPass, password2: newPass,
  }),
});
resetStore(rr);
ok(rr.status === 302 && rr.headers.get("location") === "/wire",
  `a valid reset signs in and redirects to the wire (got ${rr.status} ${rr.headers.get("location")})`);

/* THE BUG. The session the reset just created must survive the very
   next request. It did not: it carried version 0 against a database
   that now said 1. */
const afterReset = await fetch(BASE + "/wire", { headers: { cookie: resetCookie() }, redirect: "manual" });
ok(afterReset.status === 200,
  `the session created BY the reset still works one request later (got ${afterReset.status})`);

// And the old one is gone, which is what bumping the version is for.
const oldAfter = await fetch(BASE + "/wire", { headers: { cookie: oldCookie() }, redirect: "manual" });
ok(oldAfter.status === 302,
  `the session from before the reset is rejected (got ${oldAfter.status})`);

// The new password is the one that works now.
const freshJar = new Map();
let fr = await fetch(BASE + "/signin", { headers: { cookie: "" }, redirect: "manual" });
for (const c of fr.headers.getSetCookie?.() ?? []) {
  const [pp] = c.split(";"); const i = pp.indexOf("=");
  freshJar.set(pp.slice(0, i), pp.slice(i + 1));
}
const freshCookie = () => [...freshJar].map(([k, v]) => `${k}=${v}`).join("; ");
const fh = await fr.text();
fr = await fetch(BASE + "/signin", {
  method: "POST", redirect: "manual",
  headers: { cookie: freshCookie(), "content-type": "application/x-www-form-urlencoded" },
  body: new URLSearchParams({ _csrf: csrf(fh), email: resetEmail, password: newPass }),
});
ok(fr.status === 302, "the new password signs in");

await collections.users().deleteOne({ _id: resetUser });

/* ADMIN "VERIFY BY HAND" MUST LEAVE A USABLE ACCOUNT.
 *
 * It set verified:true and cleared the OTP fields without touching
 * pendingPassHash — where a signup password sits until the mailbox is
 * proved. So it produced accounts reporting VERIFIED with passHash null
 * and the real password stranded in staging: the person could not sign
 * in, and the button meant to rescue a locked account finished locking
 * it. Two definitions of "verified" is one too many.
 */
const staged = await pw.hash("the-staged-password");
const stagedUser = (await collections.users().insertOne({
  email: `e2e-staged-${Date.now()}@example.invalid`,
  verified: false, pendingPassHash: staged, passHash: null,
  otpHash: "x", otpExpiresAt: new Date(Date.now() + 60000), otpAttempts: 0,
  createdAt: new Date(),
})).insertedId;

const UsersMod = await import("../src/models/users.js");
const completed = await UsersMod.completeVerification(stagedUser);
ok(completed.verified === true, "completeVerification marks the account verified");
ok(completed.passHash === staged,
  "AND promotes the staged password — the admin path used to skip this");

const stagedRow = await collections.users().findOne({ _id: stagedUser });
ok(!stagedRow.pendingPassHash && !stagedRow.otpHash,
  "clearing the staging and the code happens in the SAME write, so a crash cannot strand it");

/* An account with nothing staged must not have its password blanked. */
const plainUser = (await collections.users().insertOne({
  email: `e2e-plain-${Date.now()}@example.invalid`,
  verified: false, passHash: "already-set", createdAt: new Date(),
})).insertedId;
const plainDone = await UsersMod.completeVerification(plainUser);
ok(plainDone.passHash === "already-set",
