...
console.log("\n=== the scheduler: who sweeps next, and can the cadence be met ===");

/* The frozen list. findDue(10) took ten due queries once and then walked
   them; at 78-92 seconds a LinkedIn sweep that is thirteen minutes
   during which the list is stale. A query becoming due at minute two
   waited until minute thirteen, so a five-minute watch quietly became a
   fifteen-minute one with LinkedIn doing nothing wrong. */
const SCH = await import("../src/services/poller/schedule.js");

const at = (mins) => new Date(mins * 60_000);
const q = (id, everyMinutes, dueAtMin, extra = {}) => ({
  _id: id, keywords: [id], everyMinutes,
  nextFetchAt: dueAtMin == null ? null : at(dueAtMin),
  ...extra,
});

// ── selection ────────────────────────────────────────────────
const now10 = 10 * 60_000;
check("nothing due selects nothing",
  SCH.selectNext([q("a", 5, 20)], now10) === null);
check("a parked query is never selected",
  SCH.selectNext([q("parked", 5, null)], now10) === null,
  "nextFetchAt null is how a search with no watchers is retired");

/* FAIRNESS, and this is the rule worth arguing about. Ranking by RAW
   lateness treats a 5-minute watch 12 minutes late the same as an
   hourly watch 12 minutes late. The first has missed more than two of
   its own cycles; the second a fifth of one. Raw lateness would
   systematically starve the fast watches, which are the ones whose
   entire purpose is being fast. */
const mixed = [
  q("hourly", 60, 8),    // 2 minutes late  = 0.03 of its cycle
  q("fast", 5, 8),       // 2 minutes late  = 0.40 of its cycle
];
check("normalised lateness picks the fast watch over the slow one",
  SCH.selectNext(mixed, now10).query._id === "fast",
  "both are 2 minutes late; only one of them has missed a meaningful share of its cadence");

/* ...but normalised lateness ALONE eats itself. A fast watch on an
   oversubscribed lane is permanently late, so it would outrank an
   hourly watch for ever and the hourly one would never run at all. */
const starved = [
  q("fast", 5, 9),        // 1 minute late
  q("hourly", 60, -25),   // 35 minutes late — past its starvation threshold
];
const pick = SCH.selectNext(starved, now10);
check("a watch left waiting past its threshold jumps the queue",
  pick.query._id === "hourly" && pick.reason === "starving",
  `${pick.query._id} / ${pick.reason} — this is what stops the fairness rule starving anybody`);

check("the threshold scales with cadence but is bounded",
  SCH.starvationThresholdMs(5 * 60_000) === 15 * 60_000 &&
  SCH.starvationThresholdMs(60 * 60_000) === 30 * 60_000,
  "3 intervals, floor 10m, ceiling 30m — an hourly watch starves at 30m, not 3h");

// Ties go to whoever has waited longest since actually being swept.
const tied = [
  q("a", 5, 5, { lastFetchedAt: at(5) }),
  q("b", 5, 5, { lastFetchedAt: at(1) }),
];
check("identical lateness breaks on the oldest observation",
  SCH.selectNext(tied, now10).query._id === "b",
  "so two identical watches alternate rather than one always winning on document order");

check("exclusion keeps a just-failed query from being reselected in the same pass",
  SCH.selectNext(tied, now10, { exclude: new Set(["b"]) }).query._id === "a");

// ── fixed slots, and no catch-up storm ───────────────────────
/* "Every five minutes" is a claim about a grid, not about the gap after
   a crawl finishes. Adding the interval to the finish time made a
   five-minute watch run every six and a half, drifting further the
   slower the board was. */
const slot = SCH.nextSlot({
  scheduledFor: at(10), intervalMs: 5 * 60_000, now: at(11.4).getTime(),
});
check("a sweep due at 10:00 that finishes at 11:24 still targets 15:00",
  slot.at.getTime() === at(15).getTime(),
  `next slot ${slot.at.getTime() / 60_000}m — not finish + 5`);
check("and nothing was missed", slot.missed === 0);

const late = SCH.nextSlot({
  scheduledFor: at(0), intervalMs: 5 * 60_000, now: at(26).getTime(),
});
check("waking 26 minutes late owes ONE observation, not five",
  late.at.getTime() === at(30).getTime(),
  `targets ${late.at.getTime() / 60_000}m — the next FUTURE slot`);
check("and the missed slots are counted rather than queued",
  late.missed === 5,
  "a catch-up burst would aim five sweeps at the one source that answers being hammered by going quiet");

// ── capacity ─────────────────────────────────────────────────
/* A LinkedIn sweep is 78-92 seconds measured. At 80s a five-minute
   cadence costs 80/300 of the lane — 27%. Three fit; four cannot, and
   no scheduler can fix that. */
const three = [0, 1, 2].map((i) => q(`q${i}`, 5, 0, { serviceMsAvg: 80_000 }));
const u3 = SCH.utilisation(three);
check("three 80-second sweeps on a 5-minute cadence fit",
  u3.state === SCH.NEAR_CAPACITY && u3.U < 1, `U=${Math.round(u3.U * 100)}% ${u3.state}`);

const four = [0, 1, 2, 3].map((i) => q(`q${i}`, 5, 0, { serviceMsAvg: 80_000 }));
const u4 = SCH.utilisation(four);
check("four do not — and it is reported, not absorbed",
  u4.state === SCH.OVERSUBSCRIBED && u4.U > 1, `U=${Math.round(u4.U * 100)}% ${u4.state}`);
check("with the cadence the lane could actually sustain",
  u4.sustainableMinutes === 6,
  `~${u4.sustainableMinutes}m — what to tell somebody whose watch says 5`);

check("a watch on an oversubscribed lane is told the honest number",
  SCH.cadenceReality({ everyMinutes: 5 }, u4).sustainable === 6 &&
  SCH.cadenceReality({ everyMinutes: 5 }, u4).honest === false);
check("and on a healthy lane it is told what it asked for",
  SCH.cadenceReality({ everyMinutes: 5 }, u3).honest === true);

/* Unmeasured queries are excluded rather than guessed at. A default
   would make U confidently wrong in whichever direction it leaned. */
const withNew = [...three, q("brand-new", 5, 0)];
const uNew = SCH.utilisation(withNew);
check("a query with no measured service time does not move U",
  Math.abs(uNew.U - u3.U) < 0.0001 && uNew.unmeasured === 1,
  "counted separately so it is not simply invisible");

check("and a parked query claims no capacity at all",
